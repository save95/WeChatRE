package unk.com.tencent.mm.model;

import android.content.Context;

public abstract interface al
{
  public abstract al N(int paramInt);

  public abstract void R(String paramString);

  public abstract al U(String paramString);

  public abstract al V(String paramString);

  public abstract al W(String paramString);

  public abstract al X(String paramString);

  public abstract al a(String paramString1, Context paramContext, String paramString2, String paramString3, Object paramObject, int paramInt);

  public abstract void a(am paramam);

  public abstract int dd();

  public abstract Object de();

  public abstract String df();

  public abstract int dg();

  public abstract String dh();

  public abstract String dj();

  public abstract void dk();

  public abstract boolean dl();

  public abstract void dn();

  public abstract String getTitle();

  public abstract void pause();

  public abstract void release();

  public abstract void resume();
}

/* Location:           /home/danghvu/0day/WeChat/WeChat_4.5_dex2jar.jar
 * Qualified Name:     com.tencent.mm.model.al
 * JD-Core Version:    0.6.2
 */