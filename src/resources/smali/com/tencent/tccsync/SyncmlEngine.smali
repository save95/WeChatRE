.class public Lcom/tencent/tccsync/SyncmlEngine;
.super Ljava/lang/Object;
.source "SourceFile"


# static fields
.field public static final TCC_ERR_NONE:I = 0x0

.field public static final TCC_ERR_SYNC_ALERT_FAIL:I = -0x7dd9

.field public static final TCC_ERR_SYNC_DEVICE_FULL:I = -0x7ddc

.field public static final TCC_ERR_SYNC_INVALID_CREDENTIALS:I = -0x7dd7

.field public static final TCC_ERR_SYNC_LARGE_OBJECT_ERROR:I = -0x7ddb

.field public static final TCC_ERR_SYNC_LARGE_OBJECT_SIZE_MISMATCH:I = -0x7dda

.field public static final TCC_ERR_SYNC_LOGIC:I = -0x7ddd

.field public static final TCC_ERR_SYNC_TARGET_MISMATCH:I = -0x7dd6

.field public static final TCC_SYNC_CRED_BASE64:I = 0x0

.field public static final TCC_SYNC_CRED_MD5:I = 0x1

.field public static final TCC_SYNC_CRED_QQMAIL:I = 0x2

.field public static final TCC_SYNC_CRYPT_NONE:I = 0x0

.field public static final TCC_SYNC_CRYPT_SSL:I = 0x20

.field public static final TCC_SYNC_CRYPT_XXTEA:I = 0x10

.field public static final TCC_SYNC_DEFLATE:I = 0x100

.field public static final TCC_SYNC_GZIP:I = 0x200

.field public static final TCC_URL_OPTION_WLAN:I = 0x2000


# instance fields
.field private cppInstance:I


# direct methods
.method static constructor <clinit>()V
    .locals 2

    .prologue
    .line 171
    const/4 v0, 0x0

    invoke-static {v0}, Lcom/tencent/qqpim/dao/ConfigDao;->getInstance(Landroid/content/Context;)Lcom/tencent/qqpim/interfaces/IConfigDao;

    move-result-object v0

    sget-object v1, Lcom/tencent/qqpim/dao/ConfigDao;->LIB_NAME:Ljava/lang/String;

    invoke-interface {v0, v1}, Lcom/tencent/qqpim/interfaces/IConfigDao;->loadLibrary(Ljava/lang/String;)Z

    .line 5
    return-void
.end method

.method public constructor <init>()V
    .locals 1

    .prologue
    .line 53
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 55
    invoke-direct {p0}, Lcom/tencent/tccsync/SyncmlEngine;->newSyncmlEngineInstance()I

    move-result v0

    iput v0, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    .line 56
    iget v0, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    if-nez v0, :cond_0

    .line 57
    new-instance v0, Ljava/lang/OutOfMemoryError;

    invoke-direct {v0}, Ljava/lang/OutOfMemoryError;-><init>()V

    throw v0

    .line 58
    :cond_0
    return-void
.end method

.method private native addDataSource(IILcom/tencent/tccsync/ITccSyncDbAdapter;Ljava/lang/String;)I
.end method

.method private native asyncNext(I)[B
.end method

.method private native asyncStart(I)[B
.end method

.method private native deleteSyncmlEngineObject(I)V
.end method

.method private native end(I)V
.end method

.method private native getLastError(I)I
.end method

.method private native getPostUrl(I)Ljava/lang/String;
.end method

.method private native newSyncmlEngineInstance()I
.end method

.method private native setConfigure(ILjava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Lcom/tencent/tccsync/RemoteSyncObserver;II)I
.end method

.method private throwIfError(I)V
    .locals 1
    .parameter

    .prologue
    .line 156
    sparse-switch p1, :sswitch_data_0

    .line 165
    new-instance v0, Lcom/tencent/tccsync/SyncException;

    invoke-direct {v0, p1}, Lcom/tencent/tccsync/SyncException;-><init>(I)V

    throw v0

    .line 162
    :sswitch_0
    new-instance v0, Ljava/lang/OutOfMemoryError;

    invoke-direct {v0}, Ljava/lang/OutOfMemoryError;-><init>()V

    throw v0

    .line 159
    :sswitch_1
    return-void

    .line 156
    :sswitch_data_0
    .sparse-switch
        -0x4 -> :sswitch_0
        0x0 -> :sswitch_1
    .end sparse-switch
.end method

.method private native writeBackRecvBuf(I[B)V
.end method


# virtual methods
.method public addDataSource(Lcom/tencent/tccsync/SyncmlEngine$SyncType;Lcom/tencent/tccsync/ITccSyncDbAdapter;Ljava/lang/String;)V
    .locals 2
    .parameter
    .parameter
    .parameter

    .prologue
    .line 97
    iget v0, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    invoke-virtual {p1}, Lcom/tencent/tccsync/SyncmlEngine$SyncType;->toInt()I

    move-result v1

    invoke-direct {p0, v0, v1, p2, p3}, Lcom/tencent/tccsync/SyncmlEngine;->addDataSource(IILcom/tencent/tccsync/ITccSyncDbAdapter;Ljava/lang/String;)I

    move-result v0

    .line 98
    invoke-direct {p0, v0}, Lcom/tencent/tccsync/SyncmlEngine;->throwIfError(I)V

    .line 99
    return-void
.end method

.method public asyncNext()[B
    .locals 1

    .prologue
    .line 123
    iget v0, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    invoke-direct {p0, v0}, Lcom/tencent/tccsync/SyncmlEngine;->asyncNext(I)[B

    move-result-object v0

    return-object v0
.end method

.method public asyncStart()[B
    .locals 1

    .prologue
    .line 111
    iget v0, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    invoke-direct {p0, v0}, Lcom/tencent/tccsync/SyncmlEngine;->asyncStart(I)[B

    move-result-object v0

    return-object v0
.end method

.method public end()V
    .locals 1

    .prologue
    .line 142
    iget v0, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    invoke-direct {p0, v0}, Lcom/tencent/tccsync/SyncmlEngine;->end(I)V

    .line 143
    iget v0, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    if-eqz v0, :cond_0

    .line 144
    iget v0, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    invoke-direct {p0, v0}, Lcom/tencent/tccsync/SyncmlEngine;->deleteSyncmlEngineObject(I)V

    .line 145
    const/4 v0, 0x0

    iput v0, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    .line 147
    :cond_0
    return-void
.end method

.method protected finalize()V
    .locals 1

    .prologue
    .line 62
    iget v0, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    if-eqz v0, :cond_0

    .line 63
    iget v0, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    invoke-direct {p0, v0}, Lcom/tencent/tccsync/SyncmlEngine;->deleteSyncmlEngineObject(I)V

    .line 64
    const/4 v0, 0x0

    iput v0, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    .line 66
    :cond_0
    return-void
.end method

.method public getLastError()I
    .locals 1

    .prologue
    .line 151
    iget v0, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    invoke-direct {p0, v0}, Lcom/tencent/tccsync/SyncmlEngine;->getLastError(I)I

    move-result v0

    return v0
.end method

.method public getPostUrl()Ljava/lang/String;
    .locals 1

    .prologue
    .line 128
    iget v0, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    invoke-direct {p0, v0}, Lcom/tencent/tccsync/SyncmlEngine;->getPostUrl(I)Ljava/lang/String;

    move-result-object v0

    return-object v0
.end method

.method public setConfigure(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Lcom/tencent/tccsync/RemoteSyncObserver;II)V
    .locals 9
    .parameter
    .parameter
    .parameter
    .parameter
    .parameter
    .parameter
    .parameter

    .prologue
    .line 83
    iget v1, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    move-object v0, p0

    move-object v2, p1

    move-object v3, p2

    move-object v4, p3

    move-object v5, p4

    move-object v6, p5

    move v7, p6

    move/from16 v8, p7

    invoke-direct/range {v0 .. v8}, Lcom/tencent/tccsync/SyncmlEngine;->setConfigure(ILjava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Lcom/tencent/tccsync/RemoteSyncObserver;II)I

    move-result v0

    .line 84
    invoke-direct {p0, v0}, Lcom/tencent/tccsync/SyncmlEngine;->throwIfError(I)V

    .line 85
    return-void
.end method

.method public writeBackRecvBuf([B)V
    .locals 1
    .parameter

    .prologue
    .line 137
    iget v0, p0, Lcom/tencent/tccsync/SyncmlEngine;->cppInstance:I

    invoke-direct {p0, v0, p1}, Lcom/tencent/tccsync/SyncmlEngine;->writeBackRecvBuf(I[B)V

    .line 138
    return-void
.end method
